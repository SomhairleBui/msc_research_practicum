...
def main():
    ...
    for month in range(1, 13):
        print(dt.now(), 'MONTH:', month, flush=True)
        ...
        df = df.astype({'stop_sequence': 'int64', 'dwell_time': 'int64'})

        # Did the bus start from stationary?
        df['dwelled'] = 0
        condition = ((df['stop_sequence'] == 2) |
                     ((df['stop_sequence'] > df['stop_sequence'].shift(1))
                      & (df['dwell_time'].shift(1) > 0)))
        df.loc[condition, 'dwelled'] = 1
        ...
        # Remove superfast buses > 80kph.
        df = df[df['hop_dist'] / df['hop_time'] < 22.22]
        ...
        df.drop(columns=['date', 'actual_arrival_time', 'arrival_code',
                         'weather_main', 'stop_id', 'prev_stop'], inplace=True)
        print('Saving:', df.shape[0], flush=True)
        df.to_csv(f'./months/{month}.csv.gz', index=False, compression='gzip')
...
def encode_scale():
    ...
    filenames = [name for name in glob('./months/*') if not os.path.isdir(name)]
    first = True
    for filename in filenames:
        print(filename, flush=True)
        df = pd.read_csv(f'{filename}', dtype='string')
        df = df[['line_id', 'weather_desc']]
        df.drop_duplicates(inplace=True)
        if first:
            df_prev = df
            first = False
    ...
    encoder.fit(df_prev[['line_id', 'weather_desc']])
    ...
    n_train_rows = 0
    for month in range(1, 13):
        print('Processing:', month, flush=True)
        df = pd.read_csv(f'./months/{month}.csv.gz', dtype='string')
        df[['line_id', 'weather_desc']] = encoder.transform(df[['line_id', 'weather_desc']])
        df.to_csv(f'./months/ordinal/{month}.csv.gz', index=False, compression='gzip')
        y = pd.DataFrame(df.pop('hop_time'))

        # Fitting scalers
        if month <= 10:
            n_train_rows += df.shape[0]
            scaler_x_train.partial_fit(df)
            scaler_y_train.partial_fit(y)
        elif month == 11:
            n_val_rows = df.shape[0]
            scaler_x_val.partial_fit(df)
            scaler_y_val.partial_fit(y)
